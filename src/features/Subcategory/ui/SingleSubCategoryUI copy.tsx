...
function SingleSubcategoryUi() {
	...
	return (
		<div ref={ref} className='flex flex-col gap-3 max-w-[80%] mx-auto pb-[5rem]'>
			...
			<ItemWrapper title={`Subcategory`} extraTitle={get(data, "id", "")}>
				<SimpleTable
					data={[
						...
						{
							title: "CategoryId",
							value: dayjs(get(data, "category.id", "no data")).format("DD.MM.YYYY HH:MM"),
						},
						{
							title: "CreatedAt",
			...
			<ItemWrapper
				title={`Category`}
				extraComponent={<EditBtn onClick={() => onModalOpen({ type: "update_subcategory", data })}>Edit</EditBtn>}
			>
				<SimpleTable
					data={[
						{
							title: "ID",
							value: get(data.category, "id", "no data"),
						},
						{
							title: "Title",
							value: get(data.category, "title", "no data"),
						},
						{
							title: "CreatedAt",
							value: dayjs(get(data.category, "createdAt", "no data")).format("DD.MM.YYYY HH:MM"),
						},
						{
							title: "UpdatedAt",
							value: dayjs(get(data.category, "updatedAt", "no data")).format("DD.MM.YYYY HH:MM"),
						},
					]}
				/>
			</ItemWrapper>
